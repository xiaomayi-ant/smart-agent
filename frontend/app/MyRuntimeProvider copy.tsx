...
import { createThread, sendMessage, updateState } from "@/lib/chatApi";
    ...
    stream: async (messages: LangChainMessage[]) => {
      ...
      try {
        ...
        if (completedAttachments.length > 0) {
          ...
          console.log(`[STREAM] 收到流式响应:`, streamResponse);
          console.log(`[STREAM] 流式响应类型:`, typeof streamResponse);
          console.log(`[STREAM] 流式响应构造函数:`, streamResponse?.constructor?.name);
          console.log(`[STREAM] 流式响应是否可迭代:`, streamResponse && typeof streamResponse[Symbol.asyncIterator] === 'function');
          console.log(`[STREAM] 流式响应是否生成器:`, streamResponse && streamResponse.constructor?.name === 'AsyncGenerator');
          
          // 将@langchain/langgraph-sdk的流式响应转换为@assistant-ui/react-langgraph期望的格式
          const convertToLangGraphFormat = async function* () {
            try {
              ...
              for await (const chunk of streamResponse) {
                ...
                console.log(`[STREAM] chunk完整内容:`, JSON.stringify(chunk, null, 2));
                console.log(`[STREAM] chunk详情:`, {
                  event: chunk?.event,
                  dataType: typeof chunk?.data,
                  dataLength: Array.isArray(chunk?.data) ? chunk.data.length : 'N/A',
                  dataContent: chunk?.data,
                  dataContentDetails: Array.isArray(chunk?.data) ? chunk.data.map((item: any, index: number) => ({
                    index,
                    type: item?.type,
                    id: item?.id,
                    content: item?.content,
                    contentLength: typeof item?.content === 'string' ? item.content.length : 'N/A'
                  })) : 'N/A'
                });
                ...
                if (chunk && typeof chunk === 'object') {
                  ...
                  if (chunk.event === 'messages/partial' && chunk.data && Array.isArray(chunk.data)) {
                    // 这是消息内容，直接传递，但确保每个消息都有id
                    hasYieldedContent = true;
                    const messagesWithId = chunk.data.map((msg: any, index: number) => {
                      if (msg.type === 'tool') {
                        // 将工具结果转换为AI消息
                        console.log(`[STREAM] 转换工具消息为AI消息:`, msg);
                        return {
                          id: msg.id || `msg_${Date.now()}_tool_${index}`,
                          type: 'ai',
                          content: msg.content
                        };
                      }
                  ...
                  } else if (chunk.event === 'on_tool_end' || chunk.event === 'on_chain_end') {
                    // 工具调用或链执行完成，转换为消息
                    console.log(`[STREAM] 处理工具/链事件:`, chunk);
                    ...
                    if (!hasYieldedContent) {
                      console.log(`[STREAM] 没有内容，发送完成占位符`);
                      yield { event: 'messages/partial', data: [{ 
                    ...
                    // 其他事件，尝试转换为消息
                    console.log(`[STREAM] 处理其他事件: ${chunk.event}`, chunk);
                    if (chunk.data.content || chunk.data.output) {
                      hasYieldedContent = true;
                      const content = chunk.data.content || chunk.data.output;
                      yield { event: 'messages/partial', data: [{ 
              ...
              if (!hasYieldedContent) {
                ...
                // 暂时禁用fallback机制，因为第一个请求实际上是成功的
                console.log(`[STREAM] 跳过fallback机制，直接发送默认响应`);
                yield { event: 'messages/partial', data: [{ 
          ...
          const langGraphStream = convertToLangGraphFormat();
          console.log(`[STREAM] 转换后的流式响应类型:`, typeof langGraphStream);
          console.log(`[STREAM] 转换后的流式响应构造函数:`, langGraphStream?.constructor?.name);
