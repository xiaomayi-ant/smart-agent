...
class VectorSearchManager:
    ...
    def _init_connections(self):
        ...
        try:
            ...
            print(f"[VectorSearch] Milvus connection initialized successfully")
    ...
    async def hybrid_search(self, query: str = None, publish_time: str = None, limit: int = 3) -> List[Dict[str, Any]]:
        ...
        try:
            print(f'[VectorSearch] Hybrid Search - Input: {{"query": "{query}", "publishTime": "{publish_time}", "limit": {limit}}}')
            ...
            if effective_query and publish_time:
                ...
                print("🔍 Executing hybrid search: keyword + time filtering")
                ...
                # Vector search only
                print("🔍 Executing vector search only")
                query_vector = await self.generate_embedding(effective_query)
                ...
                print("🔍 Executing metadata search only")
                year = publish_time[:4]
                month = publish_time[5:7]
                day = publish_time[8:10]
            ...
            print(f"[VectorSearch] Search completed, found {len(processed_results)} results")
