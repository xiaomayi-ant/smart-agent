...
async def mysql_simple_query_tool(
    ...
    try:
        ...
        print(f"[Simple Query Tool] Generated SQL: {sql}")
        print(f"[Simple Query Tool] Parameters: {tuple(params)}")
        rows = await db_manager.execute_query(sql, tuple(params))
        print(f"[Simple Query Tool] Query returned {len(rows)} rows")
        return {"success": True, "data": rows, "row_count": len(rows), "message": "OK"}
    except ValueError as ve:
        print(f"[Simple Query Tool] Validation error: {ve}")
        return {"success": False, "data": [], "row_count": 0, "message": f"Validation error: {ve}"}
    except Exception as e:
        print(f"[Simple Query Tool] Query failed: {e}")
        return {"success": False, "data": [], "row_count": 0, "message": f"Database error: {str(e)}"}
...
async def mysql_aggregated_query_tool(
    ...
    try:
        params: List[Any] = []
        # Validate field names (handle empty group_by)
        all_fields = fields + (group_by or [])
        for f in all_fields:
            if not validate_field_name(f):
                raise ValueError(f"Invalid field name: {f}")
        where_clause = build_conditions(conditions, params)
        ...
        # Build SELECT clause based on whether we have grouping
        if group_by:
        ...
        sql += _render_order_by(order_by)
        sql = _append_pagination(sql, params, limit, offset)
        print(f"[Aggregated Tool] Generated SQL: {sql}")
        print(f"[Aggregated Tool] Parameters: {tuple(params)}")
        rows = await db_manager.execute_query(sql, tuple(params))
        print(f"[Aggregated Tool] Query returned {len(rows)} rows: {rows}")
        return {"success": True, "data": rows, "row_count": len(rows), "message": "OK"}
    except Exception as e:
        print(f"[Aggregated Tool] Query failed: {e}")
...
async def mysql_join_query_tool(
    ...
    try:
        ...
        sql += _render_order_by(order_by)
        sql = _append_pagination(sql, params, limit, offset)
        print(f"[Join Query Tool] Generated SQL: {sql}")
        print(f"[Join Query Tool] Parameters: {tuple(params)}")
        rows = await db_manager.execute_query(sql, tuple(params))
        print(f"[Join Query Tool] Query returned {len(rows)} rows")
        return {"success": True, "data": rows, "row_count": len(rows), "message": "OK"}
    except ValueError as ve:
        print(f"[Join Query Tool] Validation error: {ve}")
        return {"success": False, "data": [], "row_count": 0, "message": f"Validation error: {ve}"}
    except Exception as e:
        print(f"[Join Query Tool] Query failed: {e}")
        return {"success": False, "data": [], "row_count": 0, "message": f"Database error: {str(e)}"}
...
async def mysql_custom_query_tool(query_draft: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        print(f"[Custom Query Tool] Generated SQL: {sql}")
        print(f"[Custom Query Tool] Parameters: {tuple(params)}")
        rows = await db_manager.execute_query(sql, tuple(params))
        print(f"[Custom Query Tool] Query returned {len(rows)} rows")
        return {"success": True, "data": rows, "row_count": len(rows), "message": "OK"}
    except ValueError as ve:
        print(f"[Custom Query Tool] Validation error: {ve}")
        return {"success": False, "data": [], "row_count": 0, "message": f"Validation error: {ve}"}
    except Exception as e:
        print(f"[Custom Query Tool] Query failed: {e}")
        return {"success": False, "data": [], "row_count": 0, "message": f"Database error: {str(e)}"} 