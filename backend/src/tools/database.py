...
async def get_company_facts(ticker: str) -> Optional[Dict[str, Any]]:
    """Get company facts by ticker"""
    query = """
    SELECT * FROM company_facts 
    WHERE ticker = %s AND is_active = 1
    ORDER BY updated_at DESC 
    LIMIT 1
    """
    results = await db_manager.execute_query(query, (ticker,))
    return results[0] if results else None


async def get_income_statements(ticker: str, limit: int = 10) -> List[Dict[str, Any]]:
    """Get income statements by ticker"""
    query = """
    ...
    LIMIT %s
    """
    return await db_manager.execute_query(query, (ticker, limit))


async def get_balance_sheets(ticker: str, limit: int = 10) -> List[Dict[str, Any]]:
    """Get balance sheets by ticker"""
    query = """
...
async def get_cash_flow_statements(ticker: str, limit: int = 10) -> List[Dict[str, Any]]:
    """Get cash flow statements by ticker"""
    query = """
